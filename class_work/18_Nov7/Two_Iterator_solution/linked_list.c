...
int cycleDetection(List * list){
  if(list->head==NULL) return 0; // for empty list
  
  struct Node * slow = list->head;
  struct Node * fast1 = list->head;
  struct Node * fast2 = list->head->next;
  /*while(slow != fast1 || slow != fast2){
    slow = slow->next;
    if(fast1==NULL || fast2==NULL){
      return 0; // final return should be return 1
    }
    fast1 = fast1->next->next;
    fast2 = fast2->next->next;    
  }  */
  //------ My code -------------------------------------
  while(fast1 !==NULL || fast2 !== NULL || slow !=NULL){
    slow= slow->next;
    if(fast2->next !=NULL) fast1= fast1->next->next;
    if(fast1->next !=NULL) fast2=fast2->next->next;
    if(slow ==  fast1 || slow == fast2) return 1;
  }
  return 0;
  //---------------------------------------------------
}
